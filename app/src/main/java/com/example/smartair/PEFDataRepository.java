...
public class PEFDataRepository {
    ...
    private PEFDataRepository() {
        rootRef = FirebaseDatabase.getInstance("https://smart-air-61888-default-rtdb.firebaseio.com/").getReference("users");
    }
    ...
    public void saveParentConfiguredPB(String childKey, float pbValue, OnSaveListener listener) {
        ...
        rootRef.child(childKey).child("pb").setValue(pbValue)
                .addOnSuccessListener(aVoid -> {
                    if (listener != null) listener.onSuccess();
                })
                .addOnFailureListener(e -> {
    ...
    public void fetchParentConfiguredPB(String childKey, OnPBFetchListener listener) {
        ...
        rootRef.child(childKey).child("pb").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
